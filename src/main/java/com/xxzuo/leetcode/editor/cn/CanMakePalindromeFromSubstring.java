...
// 
//
// Related Topics 位运算 数组 哈希表 字符串 前缀和 👍 149 👎 0
...
class Solution {
    public List<Boolean> canMakePaliQueries(String s, int[][] queries) {
        ...
        // 利用前缀和 计算 到第 N 个字符串时 字母出现的次数
        int[][] preSum = new int[arr.length + 1][26];
        ...
        for(int[] query : queries){
            ...
            int cnt = 0;
            for(int j = 0; j < 26; j++){
                int temp = preSum[right + 1][j] - preSum[left][j];
                if(temp % 2 == 1){
                    cnt++;
                }
            }
    ...
    public boolean check(char[] charArray, int left, int right, int k){
        HashMap<Character, Integer> record = new HashMap<>();
        for(int i = left; i <= right; i++){
            record.put(charArray[i], record.getOrDefault(charArray[i],0) + 1);
        }
        int cnt = 0;
